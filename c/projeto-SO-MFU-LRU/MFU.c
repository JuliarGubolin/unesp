...
typedef struct Pagina{
  char endereco[9];
  //Contador de acesso (Apenas no MFU)
  //----------------
  int cont;
  //----------------
  struct Pagina *prox;
...
int tamPag, tamMem, numPags, operacoes=0, lidas=0, escritas=0, acessadas=0, pag_n_encontrada=0, sobreposta=0, pags_usadas=0;
float falha=0;
FILE *file;
...
void nova_pag(char valor[9]){//Criar nova pagina
	...
	//Contador da página criada recebe 0
	//----------------
	atual->cont = 0;
	//----------------
	atual->prox = NULL;//Coloca a prox pag como NULL
	
	//Numero de paginas igual a 0
...
void MFU(char valor[9]){
	...
	if(pags_usadas == numPags)//Caso o numero de pags usadas seja igual ao que temos no sistema
	  primeira =  primeira->prox;//A pag com maior contador sera removida da memoria
}

bool encontrar(char valor[9]){//Encontrando a pag
	Pagina *tmp = primeira, *prev = NULL, *aux;//Comecamos do inicio da lista encadeada
	while(tmp != NULL){//Percorre a lista
        ...
        //----------------
		//Se o contador da página atual for maior que o da primeira,
		//a página apontada por tmp se torna a primeira
        if(tmp->cont >= primeira->cont){//Posiciona tmp em primeiro se tiver maior frequencia
          	aux = primeira;
          	primeira = tmp;
          	tmp->prox = aux;
        }			
...
void substituir_pag(char valor[9]){
  	MFU(valor);//Substitui chamando o MFU e informando o valor na memória
...
void limpa_memoria(){//Limpar a memoria
	Pagina *ant = NULL, *tmp = primeira;//Colocamos a primeira em temp
...
int main(){
  ...
  if(strlen(arq) > 0){//Se o tamanho do arquivo for maior que 0
		...
		while(fgets(linha, 20, file) != NULL){//Enquanto a linha for diferente de null
			...
			if(linha[9] == 'W' || linha[9] == 'w'){//Se a operacao eh escrita
				...
				if(encontrar(end_temp)){//Se foi encontrado o endereco na memoria, a gente Acrescentamos
				 acessadas++;	
				}
				else{//Senao 
	...
	printf("Tecnica de reposicao: MFU\n");
